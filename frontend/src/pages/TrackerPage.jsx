  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:'1.25rem'}}>
        {[{id:'all',label:'All',icon:'📋'},{id:'income',label:'Income',icon:'💵'},{id:'recurring',label:'Recurring',icon:'🔁'},...CATEGORIES].map(c=>(
      ...
      {(() => {
        ...
        return (
          <div className="card card-sm" style={{marginBottom:'1.25rem'}}>
            <div style={{fontSize:12,fontWeight:500,color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.3px',marginBottom:10}}>🔁 Recurring this month</div>
              ...
              {recurrings.map(e=>{
                ...
                return (
                  <div key={e._id} style={{display:'flex',alignItems:'center',gap:6,background:'var(--surface2)',borderRadius:8,padding:'5px 10px',fontSize:13}}>
                    ...
                    <button className="btn-xs btn btn-ghost" onClick={()=>{setEditing(e);setShowModal(true);}}>✏️</button>
                  </div>
                );
              })}
            </div>
        ...
        groups.length === 0 ? (
          ...
          return (
            <div key={date} style={{marginBottom:'1.25rem'}}>
              ...
              {exps.map(e => {
                const cat = CAT_MAP[e.category] || { icon:'📦', label: e.category, color:'#888' };
                  ...
                  <div key={e._id} className="exp-row">
                    ...
                    <div className="exp-info">
                      <div className="exp-name">
                        {cat.label}
                        {e.isRecurring && <span style={{marginLeft:6,fontSize:10,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:4,padding:'1px 5px',color:'var(--text3)'}}>🔁</span>}
                      </div>
                    ...
                    <div style={{display:'flex',gap:4,marginLeft:8}}>
                      <button className="btn btn-xs btn-ghost" onClick={()=>{setEditing(e);setShowModal(true);}}>✏️</button>
                      <button className="btn btn-xs btn-danger" onClick={()=>handleDelete(e._id)}>🗑</button>
                    </div>
                  </div>
                );
              })}
